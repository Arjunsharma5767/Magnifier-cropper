...
function endSelection() {
  ...
  const width = Math.abs(parseInt(selectionBox.style.width));
  const height = Math.abs(parseInt(selectionBox.style.height));
  
  // Use a smaller minimum size threshold (or remove it entirely)
  if (width > 0 && height > 0) {
    // Keep the selection visible and active
    selectionBox.style.display = 'block';
    selectionActive = true;
  } else {
    // Only hide if truly zero dimensions
    selectionBox.style.display = 'none';
    selectionActive = false;
  }
}
